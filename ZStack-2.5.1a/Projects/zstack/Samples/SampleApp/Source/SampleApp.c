...
void SampleApp_Send_P2P_Message( void )
{
    ...
    strData[0] = '#';   //数据中设备号开始符号
    strData[1] = ((DEVICEID >> 8) & 0xFF);
    strData[2] = (DEVICEID & 0xFF);
    strData[3] = '$';   //数据中传感器类型开始符号
    strData[4] = ((SENSERTYPEID >> 8) & 0xFF);
    strData[5] = (SENSERTYPEID & 0xFF);
    strData[6] = '*';   //数据中传感器类型开始符号

    strData[7] =wendu_shi*10+wendu_ge;
    strData[8] ='*';
